...
- [ ] 4. Implement and test the BadgeContract (Foundry)
  - [~] 4.1 Scaffold the Foundry workspace
  ...
  - [~] 4.2 Implement `BadgeContract.sol`
  ...
  - [ ]* 4.3 Foundry fuzz test: createCampaign storage round-trip
  ...
  - [ ]* 4.4 Foundry fuzz test: soulbound transfer enforcement
  ...
  - [~] 4.6 Export ABI + address consts to the frontend
...
- [~] 5. Checkpoint — contract verified
  - Run `forge test -vvv` in `contracts/`. Ensure both fuzz suites pass. Ask the user if questions arise.

- [ ] 6. Implement the server layer (database, API routes)
  - [~] 6.1 Define Drizzle schema and migrations
  ...
  - [~] 6.4 Implement `POST /api/campaigns` and `GET /api/campaigns`
...
- [ ] 7. Implement frontend providers, wallet, and network UX
  - [~] 7.1 Implement `Providers`
  ...
  - [~] 7.2 Implement `WalletButton` and `useNetworkGuard`
  ...
  - [~] 7.5 Implement disconnect cache invalidation
    - In `Providers`, listen to wagmi's account-change events and call `queryClient.removeQueries({ queryKey: ['wallet'] })` on disconnect.
    - _Requirements: 4.5_

- [ ] 8. Implement Mock USD balance and faucet flow
  - [~] 8.1 Implement `useMockUsdBalance`
  ...
  - [~] 8.2 Implement `MockUsdBalance` component with faucet button
...
- [ ] 9. Implement the UGF pipeline panel and hooks
  - [~] 9.1 Implement `UgfPipelinePanel`
  ...
  - [~] 9.3 Implement `useUgfCreateCampaign`
...
- [ ] 10. Implement claim flow components
  - [~] 10.1 Implement `useCampaign`
  ...
  - [~] 10.2 Implement `OnboardingSteps`
  ...
  - [~] 10.4 Implement `/claim/[campaignId]/page.tsx`
...
- [ ] 11. Implement creator dashboard and form
  - [~] 11.1 Implement `CampaignForm`
  ...
  - [~] 11.4 Implement `CampaignCard` and creator dashboard list
...
- [ ] 12. Implement discovery and profile views
  - [~] 12.1 Implement `DiscoveryGrid`
  ...
  - [~] 12.5 Implement `/me/page.tsx` and `/profile/[address]/page.tsx`
...
- [ ] 13. Wire global header and error UX
  - [~] 13.1 Implement `app/layout.tsx`
    - Wrap children in `Providers`. Add a header: app name, `WalletButton`, `MockUsdBalance`, links to `/`, `/creator`, `/me`.
    - _Requirements: 3.1, 4.1_

  - [~] 13.2 Implement central `mapErrorToUiMessage`
...
- [~] 14. Checkpoint — happy paths run locally
